#!/usr/bin/env python3
"""
extract_stats.py - L0 代码预处理（语音主播版）
从 reports/ 下所有 *_六维分析报告.md 中提取六维度评分、路径信号、心之壁等结构化数据，
输出 l0_raw.json 和 l0_stats.json。

与真人版的主要差异：
- 内容类型分类（ASMR/电台-情感/有声书-朗读/音乐/游戏语音/陪聊-树洞/角色扮演/知识-播客/综合）
- 组织属性（音频平台签约/公会-MCN签约/独立/声优艺人转型）
- 新增语音特有字段：声带健康风险、声音匿名性、声线偏离度、③近恒等程度
- 无外貌管理压力字段（语音主播不适用）
"""
...
def parse_path_signals(text: str) -> dict:
    signals = {}
    section = None
    for heading in ['17条关系路径信号表', '18条关系路径信号表', '关系路径信号表', '路径信号表', '关系路径信号']:
...
def parse_heart_wall(text: str) -> dict:
    ...
    for row in rows:
        ...
        if '类型' in key or '壁类型' in key or '壁' in key:
            result['type'] = val
            if '声音匿名' in val or '匿名' in val:
                result['wall_category'] = '声音匿名型'
            elif '机构' in val or '约束' in val or '平台' in val:
    ...
    if result['type'] == '未知':
        ...
        if not m:
            m = re.search(r'(声音匿名型|机构约束型|外力打破|内部崩溃|自发管理型|极厚|厚壁|中偏厚|薄壁)', section)
        if m:
            result['type'] = m.group(1).strip()
            val = result['type']
            if '匿名' in val:
                result['wall_category'] = '声音匿名型'
            elif '机构' in val or '约束' in val:
                result['wall_category'] = '机构约束型'
...
def parse_voice_special_factors(text: str) -> dict:
    """Extract 语音主播特殊因子 section."""
    factors = {
        'vocal_health_risk': '未知',
        'anonymity_strength': '未知',
        'voice_recognizability': '未知',
        'listener_image_gap': '未知',
        'near_identity_3': '未知',  # ③近恒等程度
        'voice_deviation': '未知',  # 声线偏离度
    }
    section = extract_section(text, '语音主播特殊因子')
    if not section:
        return factors

    if '声带健康' in section:
        if '高' in section:
            factors['vocal_health_risk'] = '高'
        elif '中' in section:
            factors['vocal_health_risk'] = '中'
        elif '低' in section:
            factors['vocal_health_risk'] = '低'

    if '匿名性' in section:
        if '强' in section:
            factors['anonymity_strength'] = '强'
        elif '中' in section or '弱' in section:
            factors['anonymity_strength'] = '中' if '中' in section else '弱'
        if '打破' in section:
            factors['anonymity_strength'] = '已被打破'

    if '辨识度' in section:
        if '高' in section:
            factors['voice_recognizability'] = '高'
        elif '中' in section:
            factors['voice_recognizability'] = '中'
        elif '低' in section:
            factors['voice_recognizability'] = '低'

    if '脑补' in section or '形象偏差' in section:
        if '大' in section:
            factors['listener_image_gap'] = '大'
        elif '中' in section:
            factors['listener_image_gap'] = '中'
        elif '小' in section:
            factors['listener_image_gap'] = '小'

    if '③' in section or '近恒等' in section:
        if '近恒等' in section:
            factors['near_identity_3'] = '近恒等'
        elif '高度重叠' in section:
            factors['near_identity_3'] = '高度重叠'
        elif '部分重叠' in section:
            factors['near_identity_3'] = '部分重叠'

    if '声线偏离' in section or '伪声' in section or '变声' in section:
        if '伪声' in section or '变声器' in section:
            factors['voice_deviation'] = '伪声/变声器'
        elif '修饰' in section:
            factors['voice_deviation'] = '轻度修饰'
        elif '自然' in section:
            factors['voice_deviation'] = '自然声线'

    return factors


# ============================================================
# 3. Metadata Extraction (语音主播版)
# ============================================================
    ...
    for k, v in info.items():
        if '地域' in k:
            ...
            if '韩国' in v or 'KR' in v.upper() or 'Korea' in v:
                return '韩国'
            if '印尼' in v or '泰国' in v or '东南亚' in v or 'SEA' in v.upper():
                return '东南亚及其他'
    ...
    for k, v in info.items():
        if '时长' in k or '从业' in k:
            ...
            if '超长' in v or '5年' in v or '元老' in v:
                return '超长待机'
            if '停播' in v or '淡出' in v or '引退' in v:
                return '停播淡出'
    return '未分类'


def extract_type_from_info(info: dict) -> str:
    """Extract content type (语音主播版分类)."""
    for k, v in info.items():
        if '类型' in k or '内容' in k:
            v = v.strip()
            if 'ASMR' in v.upper() or 'asmr' in v.lower() or '助眠' in v or '耳语' in v:
                return 'ASMR'
            if '电台' in v or '情感' in v or 'radio' in v.lower() or '深夜' in v:
                return '电台/情感'
            if '有声书' in v or '朗读' in v or 'audiobook' in v.lower() or 'reading' in v.lower():
                return '有声书/朗读'
            if '音乐' in v or '唱歌' in v or '歌' in v or 'music' in v.lower() or 'sing' in v.lower():
                return '音乐'
            if '游戏' in v or 'game' in v.lower() or 'gaming' in v.lower():
                return '游戏语音'
            if '陪聊' in v or '树洞' in v or '倾诉' in v or 'chat' in v.lower():
                return '陪聊/树洞'
            if '角色扮演' in v or '音声' in v or '声线切换' in v or 'roleplay' in v.lower():
                return '角色扮演'
            if '知识' in v or '播客' in v or 'podcast' in v.lower() or 'educational' in v.lower():
                return '知识/播客'
            if '综合' in v or 'mixed' in v.lower() or 'variety' in v.lower():
                return '综合'
    return '综合'
...
def extract_state_from_info(info: dict) -> str:
    for k, v in info.items():
        if '状态' in k or '从业' in k:
            v = v.strip()
            if '停播' in v or '淡出' in v or '引退' in v or 'hiatus' in v.lower():
                return '停播淡出'
            if '瓶颈' in v or '倦怠' in v or 'burnout' in v.lower():
                return '瓶颈'
            if '活跃' in v or 'active' in v.lower():
                return '活跃'
    return '活跃'


def extract_org_from_info(info: dict) -> str:
    """Extract organization type (语音主播版)."""
    for k, v in info.items():
        if '组织' in k or '所属' in k or 'organization' in k.lower():
            v = v.strip()
            if '声优' in v or '艺人' in v or '配音' in v:
                return '声优/艺人转型'
            if '独立' in v or '个人' in v or 'indie' in v.lower():
                return '独立'
            if '平台签约' in v or '独家' in v or 'exclusive' in v.lower():
                return '音频平台签约'
            if '公会' in v or 'MCN' in v.upper() or 'mcn' in v.lower() or '签约' in v:
                return '公会/MCN签约'
    return '独立'
...
def extract_scale_from_info(info: dict) -> str:
    for k, v in info.items():
        if '规模' in k or '粉丝' in k or '听众' in k or '订阅' in k or 'subscriber' in k.lower():
            v = v.strip()
            ...
            if nums_k:
                ...
                if n >= 1:
                    return '中小层'
                return '底层/新人'
            if '顶级' in v or '100万' in v:
                return '顶级'
            if '头部' in v or '30万' in v:
                return '头部'
            if '腰部' in v or '5万' in v:
                return '腰部'
            if '中小' in v or '5000' in v:
                return '中小层'
            if '底层' in v or '新人' in v:
                return '底层/新人'
    return '未分类'
...
def extract_one_report(filepath: Path) -> dict | None:
    ...
    voice_factors = parse_voice_special_factors(text)
    ...
    result = {
        ...
        'voice_special_factors': voice_factors,
        '_parse_quality': {
...
def compute_stats(raw_data: list[dict]) -> dict:
    ...
    def desc_stats(values: list[float]) -> dict:
        ...
        return {'mean': round(mean, 2), 'median': round(median, 2), 'std': round(std, 2), 'min': min(values), 'max': max(values), 'count': n}
    ...
    def group_stats(data: list[dict], group_key: str) -> dict:
        ...
        for g, items in sorted(groups.items()):
            ...
            result[g] = {'count': len(items), 'dimensions': dim_stats}
        return result
    ...
    stats['full_sample'] = {'total_reports': len(raw_data), 'valid_reports': len(valid), 'dimensions': full_dim_stats}
    ...
    # Voice special factors
    vocal_health = Counter(d.get('voice_special_factors', {}).get('vocal_health_risk', '未知') for d in valid)
    anonymity = Counter(d.get('voice_special_factors', {}).get('anonymity_strength', '未知') for d in valid)
    voice_dev = Counter(d.get('voice_special_factors', {}).get('voice_deviation', '未知') for d in valid)
    near_identity = Counter(d.get('voice_special_factors', {}).get('near_identity_3', '未知') for d in valid)
    stats['voice_special_factors'] = {
        'vocal_health_risk': dict(vocal_health),
        'anonymity_strength': dict(anonymity),
        'voice_deviation': dict(voice_dev),
        'near_identity_3': dict(near_identity),
    }
        ...
        for dim in dims:
            ...
            for d, s in vals:
                ...
                if abs(z) > 1.5 or abs(s - mean) >= 2:
                    outliers.append({
                        'name': d['name'], 'dimension': dim, 'score': s,
                        'group': region, 'group_mean': round(mean, 2),
                        'z_score': round(z, 2), 'deviation': round(s - mean, 2)
                    })
        ...
        for r2 in regions_list[i+1:]:
            for dim in dims:
                ...
                if v1 is not None and v2 is not None:
                    diff = round(v1 - v2, 2)
                    if abs(diff) >= 1.0:
                        differences.append({'region1': r1, 'region2': r2, 'dimension': dim, 'diff': diff})
...
def generate_report_index(raw_data: list[dict], output_path: Path):
    lines = []
    lines.append('# 六维分析报告索引（语音主播）\n')
    lines.append(f'> 索引生成时间：2026-06-25\n')
    lines.append(f'> 报告总数：{len(raw_data)} 份\n')
    lines.append('')
    lines.append('| 序号 | 主播名 | 地域 | 状态 | 时长 | 类型 | 规模 | 组织 | 心之壁 | 声带风险 | 匿名性 | 人设 | 互动 | 粉丝 | 营收 | 状态 | 内容 | 报告链接 |')
    lines.append('|------|--------|------|------|------|------|------|------|------|:---:|:---:|------|------|------|------|------|------|---------|')
    for i, d in enumerate(raw_data):
        ...
        vf = d.get('voice_special_factors', {})
        vocal = vf.get('vocal_health_risk', '—')
        anon = vf.get('anonymity_strength', '—')
        scores = d.get('scores', {})
        ...
        link = f'[报告](./{filename})'
        lines.append(f'| {i+1} | {name} | {region} | {state} | {tenure} | {ctype} | {scale} | {org} | {hw} | {vocal} | {anon} | {s1} | {s2} | {s3} | {s4} | {s5} | {s6} | {link} |')
...
def main():
    print("=" * 60)
    print("Phase 3 L0: 代码预处理(语音主播版) - 从报告中提取结构化数据")
    print("=" * 60)
    print("\n[1/4] 提取报告数据...")
    ...
    print(f"  有效报告数: {full.get('valid_reports', 0)}")
    dims_stats = full.get('dimensions', {})
    for dim, ds in dims_stats.items():
        if ds.get('mean') is not None:
            print(f"  {dim}: 均值={ds['mean']}, 中位数={ds['median']}, std={ds['std']}")
    vsf = stats.get('voice_special_factors', {})
    if vsf:
        print(f"\n  === 语音主播特殊因子 ===")
        print(f"  声带健康风险: {vsf.get('vocal_health_risk', {})}")
        print(f"  声音匿名性: {vsf.get('anonymity_strength', {})}")
        print(f"  声线偏离度: {vsf.get('voice_deviation', {})}")
        print(f"  ③近恒等程度: {vsf.get('near_identity_3', {})}")
    print("\n[4/4] 生成 00_报告索引.md...")
